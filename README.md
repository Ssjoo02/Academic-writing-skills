...
**Academic Writing** 是一个专注于学术论文写作的 skill 集合，用于生成符合具体会议或期刊格式要求的完整论文初稿，可在 Claude Code、Codex 等本地 skill 环境中使用。
...
安装时整仓被拷贝到 `.../skills/academic-writing/`，环境据**仓库根 `SKILL.md`** 发现并启动本集合。
...
请复制**整个仓库**（包含 `skills/` 与根目录的 `_shared/`），不要只复制单个 `SKILL.md` 或单个子 skill 目录，因为各子 skill 通过相对路径依赖 `_shared/`（核心 stance/gates/contract、venue 模板、paper-type/venue 卡片、共享 check 规则）。复制整个仓库后，四个 skill（`academic-writing`、`academic-figure`、`academic-citation`、`academic-review`）会一起可用。
