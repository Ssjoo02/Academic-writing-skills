...
Do not show internal artifacts by default. Show them only when the user asks, when the task is
explicitly diagnostic/review-oriented, or when a blocking evidence/citation risk must be surfaced.
...
For whole-paper review: concise findings ordered by severity, required revision, evidence/citation
risks.
