...
Figures/Tables, paragraph flow) use the base guide directly. The journal posture
for those is covered by `references/venues/journal-vs-conference.md` (complete,
self-contained story; thorough evaluation; written to survive multi-round review)
and the active venue card.

## Cross-References

- `references/venues/journal-vs-conference.md` — overall journal drafting posture.
- `references/checks/journal-submission-elements.md` — mandatory statements,
  display-item caps and tiers, methods placement, and word-budget mechanics that
  are journal-wide rather than per-section.
- The active venue card (`jmlr.md`, `ieee-tpami.md`, `journal-generic.md`) — the
