...
4. **stop-and-ask** — when the issue is decisive for paper identity, a central claim, key
   terminology, or an evidence conflict, stop and ask the user (per `static/core/gates.md`).
...
2. **Cross-file consistency.** When the same metric, count, denominator, model name, or dataset
   ...
   Conflict Rule in `static/core/contract.md`: decisive conflict → stop-and-ask; non-decisive →
...
catch confirmation bias. This reuses the Reviewer Independence rules in
`references/sections/paper-review.md` (the same model used for the Round 2 subagent there).
