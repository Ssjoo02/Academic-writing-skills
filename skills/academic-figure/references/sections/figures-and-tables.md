...
- **Use cross-column (`table*`, `\textwidth`)** only when the table genuinely needs the horizontal
  room: the **headline/main results table**, a **wide model×metric matrix**, or a table whose cells
  become unreadable in one column (e.g., wrapping to one word per line). A table with **≳5–6 numeric
  columns** triggers a span check; it does not automatically become cross-column if the table is only
  a compact secondary result. The main results table is *usually* cross-column, but verify against its
  actual density rather than promoting reflexively.
...
1. **Exceeds `\columnwidth` but fits `\textwidth` → first check whether it is body-worthy and
   full-width-justified.** If it is the main-results table or a genuine wide result matrix, make it
   cross-column `table*` filling `\textwidth` in a two-column template, or a normal `table` with
   `tabular*{\textwidth}` / `tabularx{\textwidth}` in a one-column template. If it is a compact
   secondary table, split, abbreviate, or move detail to appendix instead of stretching sparse
   content.
...
Rule of thumb: a numeric table with **≳6 columns**, or with long header text, needs a span check. It
may be full-width only when it is the main result or a genuine wide matrix; otherwise split or move
detail out of the body. A table with **≳10 columns** usually needs rotation or splitting even at full
width. Treat any compile-time overfull `\hbox` originating in a `tabular` as this defect.
...
`\textwidth`; never place those cells in plain `l`, `c`, or `r` columns. For full-width-justified
per-model / per-application numeric matrices, abbreviate model headers and use the full-width
pattern; expand full model names in the caption or lead paragraph. A single-column appendix `table`
that visibly overflows is a blocking source defect even before final PDF polish.
...
- `double-column` / `full-width`: allowed only with a span justification. In a two-column template,
  use `table*` and make the tabular span `\textwidth` with
  `\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}...}` (numeric columns, no wrapping) or
  `\begin{tabularx}{\textwidth}{...}` (with `X`/`Y` columns when cells wrap). In a one-column
  template, use a regular `table` plus `tabular*{\textwidth}` / `tabularx{\textwidth}`. If the
  content is too sparse to occupy the span cleanly, demote it to single-column.
...
- Compact metric table with short labels: normal `tabular`, optionally `\footnotesize`, bounded by
  `\linewidth`; split or move detail to appendix before using full-width for a secondary table.
